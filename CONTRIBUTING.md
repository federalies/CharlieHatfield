...
4. Reporting a Bug
5. Suggesting a New Feature/Change
...
We love incoporating new ideas into Charlie Hatfield, even though it is fairly limited in scope. We aim to be as open as reasonable, even though not all ideas really fit. If you have an idea for `CharlieHatfield` the best thing to do is run your idea by someone in the community.

It is strongly recommend to collborate on your idea so that you don't spend a lot of time on something that is a poor fit.

When contributing code or ideas ot the repository, the way to start a new discussion is to hit the [New Issue] button in Github. Friendly users have found it productive to run your change by some people in chat, and then copy/paste the thread into the newly created issue.

All community members and contributors should understand that this project has a code of connuct that spells out what appropriate and principaled behaviors should feel like. We abide by a code of conduct, which is centered on hospitatliy,aiming to be as welcome in as many new contributors as the project deserves.
...
## Setting expectations good behavior
...
Prettier + ESLint is used for checking code style. It is also useful for avoiding some errors. This project uses See the `.prettier.js` settings + `.eslintrc.js`

camelCase should be used for all variables and file names. Avoid the use of underscores as word separators. This extends to things like CSS class names, IDs in test HTML documents, everything. Some exceptions to JSLint can be indicated at the top of each JS file, but they should be avoided when possible. Two common exceptions you may see in the source:
...
[source: [cucumber-ruby](https://github.com/cucumber/cucumber-ruby/blob/master/CONTRIBUTING.md#talking-with-other-devs)] **Need more inspiration?** [1] [Chef](https://github.com/chef/chef/blob/master/CONTRIBUTING.md#-developer-office-hours) [2] [Cookiecutter](https://github.com/audreyr/cookiecutter#community)
